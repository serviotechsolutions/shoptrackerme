...
const Reports = () => {
  const [dayStats, setDayStats] = useState<PeriodStats>({
    sales: 0,
    profit: 0,
    transactions: 0
  });
  ...
  const fetchReports = async () => {
    try {
      ...
      if (allTransactions) {
        ...
        setDayStats(calculateStats(allTransactions.filter(t => new Date(t.created_at) >= todayStart)));
        setWeekStats(calculateStats(allTransactions.filter(t => new Date(t.created_at) >= weekStart)));
        setMonthStats(calculateStats(allTransactions.filter(t => new Date(t.created_at) >= monthStart)));
        setYearStats(calculateStats(allTransactions.filter(t => new Date(t.created_at) >= yearStart)));
        ...
        setProductStats(Array.from(productMap.values()).sort((a, b) => b.total_sales - a.total_sales));
      }
    } catch (error) {
  ...
  return <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-center">Track your business performance</p>
        </div>
        ...
        <Tabs defaultValue="overview" className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center p-4 rounded-lg bg-success/10">
                      ...
                      <p className="text-lg font-bold text-success">
                        {monthStats.sales > 0 ? `${(monthStats.profit / monthStats.sales * 100).toFixed(1)}%` : '0%'}
                      </p>
                    </div>
                    ...
                    <div className="text-center p-4 rounded-lg bg-primary/10">
                      ...
                      <p className="text-lg font-bold text-primary">
                        {yearStats.transactions > 0 ? formatCurrency(yearStats.sales / yearStats.transactions) : formatCurrency(0)}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <TabsContent value="products" className="space-y-4">
            <Card>
              ...
              <CardContent>
                {productStats.length > 0 ? <ResponsiveContainer width="100%" height={400}>
                    <BarChart data={productStats.slice(0, 10)}>
                      <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
                      <XAxis dataKey="product_name" className="text-xs" angle={-45} textAnchor="end" height={100} />
                  ...
                  </ResponsiveContainer> : <p className="text-center text-muted-foreground py-8">
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {productStats.length === 0 ? <TableRow>
                        <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                          No sales data available
                        </TableCell>
                      </TableRow> : productStats.map(product => <TableRow key={product.product_name}>
