...
serve(async (req) => {
  ...
  try {
    ...
    const { error: checkError } = await supabase.rpc('check_low_stock_notifications');

    if (checkError) {
      console.error('Error checking low stock:', checkError);
      throw checkError;
    }
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Low stock notifications checked' }),
      {
